...
function initCalculator() {
  ...
  document.getElementById("hn-calc").addEventListener("click", function () {
    ...
    if (mode === "yards") {
      ...
      if (selectedMaterialId === "sod") {
        ...
        const rollCoverage = 10;
        const pricePerRoll = 8.50;

        const rolls = Math.ceil(totalArea / rollCoverage);
        const totalCost = rolls * pricePerRoll;
        ...
        document.getElementById("hn-result").innerHTML = `
          ...
          <div><strong>Material:</strong> Sod</div>
          <div><strong>Area:</strong> ${totalArea.toFixed(2)} sq ft</div>

          <div style="margin-top:10px;">
            <strong>Sod Rolls Needed:</strong> ${rolls}
          </div>

          <div>
            <strong>Price per Roll:</strong> $${pricePerRoll.toFixed(2)}
          </div>
    ...
    document.getElementById("hn-result").innerHTML = `...`; // unchanged remainder
  });
  ...
  if (printBtn) {
    printBtn.onclick = function () {
      ...
      w.document.write(`<html><body>${resultHTML}</body></html>`);
      w.document.close();
      w.print();
    };
  }
...
waitForMaterials(initCalculator);    option.textContent = mat.name;
    materialDropdown.appendChild(option);
  });
